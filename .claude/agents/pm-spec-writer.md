...
3. **Team Orchestration Plan names real agents** from `.claude/agents/` (Glob to confirm) or explicitly says the agent must be created via `/new-domain-agent` first.
...
- **Route execution to the right domain expert.** Before finishing, check `.claude/agents/` and name the owner for each orchestration step. If no owner exists for a surface, recommend `/new-domain-agent` in the Team Orchestration Plan.
