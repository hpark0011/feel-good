...
| FR-06 | Tone preset options and labels are sourced exclusively from `tonePresets.ts` | Grep: no inline string `"Professional" | "Friendly"` etc. in UI files; `TonePresetSelect` imports from the shared module |
...
|-------|--------|------|-------|
| `users` | `personaPrompt` | `v.union(v.string(), v.null())` (widened from `v.optional(v.string())`) | Must be explicitly nullable so clearing can set `null` via `ctx.db.patch`. Convex `patch` silently drops `undefined` keys, so `undefined` cannot be used to unset. Data migration: existing `undefined`/absent values are still valid under `v.union(string, null)` because the column retains `v.optional` at the document level; see note below. |
