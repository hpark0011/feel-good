...
- [ ] `statusRef.current` is set to `"loading"` synchronously in `requestOTP`, `verifyOTP`, and `resendOTP` before `setStatus("loading")`
- [ ] Pasting 6 digits + pressing Enter does not produce two network requests
- [ ] `onComplete` + clicking "Verify" does not produce two network requests
